...
echo "" >> gizmo-log
echo "======================= real010 Slurm run $this_job_number, job ID $SLURM_JOBID =======================" >> gizmo-log
echo "" >> gizmo-error
echo "======================= real010 Slurm run $this_job_number, job ID $SLURM_JOBID =======================" >> gizmo-error
echo "" >> slurm-log
echo "======================= real010 Slurm run $this_job_number, job ID $SLURM_JOBID =======================" >> slurm-log
echo "" >> slurm-error
echo "======================= real010 Slurm run $this_job_number, job ID $SLURM_JOBID =======================" >> slurm-error

# start GIZMO
echo Time: $(date)
echo "Starting GIZMO for real010 on $SLURM_JOB_NUM_NODES nodes ($SLURM_NTASKS tasks). The job ID is $SLURM_JOBID."
...
if test $this_job_number -eq 1; then # starting for first time
    ...
    mpirun $HOME/bin/GIZMO_d768f3d real010.param >> gizmo-log 2>> gizmo-error
else # start from restart files
    echo "Starting GIZMO from restart files."
    mpirun $HOME/bin/GIZMO_d768f3d real010.param 1 >> gizmo-log 2>> gizmo-error
fi
echo "GIZMO done!"
